...
├── ownership-evidence/       # 本机语义活动与 writer 证据
├── instance-id               # 稳定安装实例 ID（不是 Client 名）
