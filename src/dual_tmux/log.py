...
KIND_META: dict[str, tuple[str, str, str]] = {
    ...
    "dt.rm.recreate": ("system", "info", "删后重建（墓碑作废）"),
    "dt.rm.foreign_occupancy": ("system", "warn", "删除时他机仍占用"),
    "sync.tombstone.applied": ("system", "info", "墓碑收敛（本侧副本删除）"),
    ...
    "recovery.rebuild.auto": ("system", "info", "自动重建触发"),
