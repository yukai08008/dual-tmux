...
Bindings live in `dt inspect`. Another Client: `dt pull` then `dt resume` (oc JSON still needs persist restore). Hub lock: only one Client active; `dt park` releases. `dt push` does not copy `config.toml`.
