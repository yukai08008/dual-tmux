...
- 真实 `dt-company_intro_v2` 从 free/残留 writer 场景恢复，10 秒取得 generation 18 并完成两侧启动；发布后持续观察 Lease 仍为 owned/age 0～1 秒，Trigger=`opencode`、Bullet transport=`ssh` 持续存活。全量 344 passed + 2 skipped，聚焦 75 tests、Ruff、build、data/diff 门禁全绿。PR #40 已合并至 `main`（merge `e91e817`），`v0.4.55.post2` Release 与 wheel/sdist 已发布，两条 upgrade discovery 均返回 post2。
...
- PR #39 已合并至 `main`（merge `804e6ee`）；`v0.4.55.post1`、wheel 与 sdist 已发布。本机通过 `dt upgrade` 从 0.4.55 升级并验证为 0.4.55.post1；334 passed + 2 skipped，Ruff 与 build 全绿。

### v0.4.55 (RELEASED) — Trigger 独热接管 API

- 三件套：`dev_plans/v0.4.54-v0.4.55/`。
- **feature/v0.4.55-exclusive-trigger** (MERGED): PR #37 合并至 `main`（merge `c0832a0`）。健康 handoff 保持 `persist → commit → park → atomic transfer`，并新增 Hub 服务模式故障仲裁：m7 为唯一 Ownership 权威，活动端 1 秒 watchdog / 4 秒 Lease v2，legacy 保持 300 秒兼容且可安全迁移；故障接管使用 `reservation → 服务端 writer 清场并二次确认 → generation 原子转让 → restore`，清场失败 fail-closed，旧 generation 恢复后 self-fence；Web/ControlService send 也在写 tmux 前统一验证 ownership。真实 tom7r 故障到新 Trigger INPUT_ACK 连续 5 次 6.410–8.019 秒；完整健康 `ControlService.resume` 最近 5.752/6.963/7.608 秒，旧 attach 返回 shell。默认 336 passed + 2 opt-in，聚焦 Ruff/build/data/diff 门禁全绿；`v0.4.55-final`、`v0.4.55` 与 GitHub Release 已发布，隔离安装及 API/redirect 两条 upgrade discovery 均验证为 0.4.55。本版经用户授权作为奇数 API 版安全能力例外发布。
