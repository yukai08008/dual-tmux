...
> 最近更新: 2026-09-06 14:30 +08:00 | 更新者: Codex PM
...
- **feature/v0.4.49-session-ownership** (DEVELOPING): 基于 post6 继续实现 snapshot freshness 收敛、Lease/活动分离和事务化 resume；规划及故障证据已入库。
  ...
  - **issue-enter-trigger-workdir-home** (RESOLVED, `hotfix/v0.4.49-enter-workdir`): `cmd_enter()` 现于 discover/attach 前创建并校准 trigger tmux；新 pane 固定使用 ops cwd，已有错误 cwd 只在前台为空闲 shell 时清行并纠正，Agent/其他程序不注入命令。全量 pytest 通过；`dt-cp-gate` 实测 pane 与 `op_point.cwd` 均已收敛到 `/Users/andy/.dual-tmux/ops/op_cp_gate`。
