...
def test_operation_catalog_has_control_metadata():
    ...
    assert all(row["capability"] and row["risk"] and row["surfaces"] and row["audit_event"] for row in rows)
...
def test_list_get_and_send_return_structured_results(tmp_path, monkeypatch):
    ...
    sent = []
    monkeypatch.setattr("dual_tmux.control.tmux_ops.send_keys", lambda pane, text: sent.append((pane, text)))
...
def test_control_wraps_legacy_freeze_resume_and_model(monkeypatch):
    ...
    data = _tunnel()
    monkeypatch.setattr(cli, "_apply_freeze_legacy", lambda name, sides, tool: {**data, "call": [name, sides, tool]})
    monkeypatch.setattr(cli, "_apply_resume_legacy", lambda name, force: {**data, "call": [name, force]})
    monkeypatch.setattr(ControlService, "get_tunnel", lambda self, name: type("R", (), {"data": data})())
    monkeypatch.setattr(cli, "_apply_model_legacy", lambda name, model, sides: {**data, "call": [name, model, sides]})
    service = ControlService()
    assert service.freeze("msg", ["trigger"], "auto").data["call"] == ["msg", ["trigger"], "auto"]
    assert service.resume("msg", True).data["call"] == ["msg", True]
    assert service.model("msg", "p/m", ["trigger"]).data["call"] == ["msg", "p/m", ["trigger"]]
