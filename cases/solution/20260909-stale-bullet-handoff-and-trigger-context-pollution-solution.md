...
- 启动或复用 OpenCode Trigger 后，必须等待目标 session ID、OpenCode 进程和可识别
  footer 同时就绪，并设置有界超时；不能因 `ensure_agent` 已发送命令就立即返回成功。
...
| Trigger 尚未初始化就提前返回 | `test_resume_waits_for_trigger_session_ready` |
