...
4. Hub installation 已存在时拒绝再次扫码，必须先显式解绑/更换。
5. Client failover 从 Hub 拉取同一套加密 key+ciphertext，验证后在凭据锁内原子安装。
6. active 全局状态与逐实例 candidate 状态分离，standby 不覆盖 connected。
7. `event_id` 幂等继续作为切换窗口的最后防线。
