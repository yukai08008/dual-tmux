...
| 维度 | 数量 | 通过 | 失败 | 待执行 |
|---|---:|---:|---:|---:|
| Python 自动化 | 317 | 316 | 0 | 1（opt-in） |
| 独热聚焦测试 | 53 | 52 | 0 | 1（opt-in） |
...
| ID | 用例 | 结果 | 备注 |
|---|---|---|---|
| B-01 | 全量 Python 回归 | PASS | 默认门禁 `316 passed, 1 skipped`；opt-in Hub 用例另行通过 |
...
| D-02 | 双隔离 Client 显式 handoff | PASS | 真实 daemon/tmux/PTY/persist，两次复跑 6.771 / 5.605 秒；旧 attach 返回 shell |
